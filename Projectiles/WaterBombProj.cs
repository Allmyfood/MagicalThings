...
{
    public class WaterBombProj : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Water Bomb Explosion");
        }
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 58; i++)
            {
                ...
                Dust.NewDust(projectile.position, projectile.width, projectile.height, 252, speedX, speedY, 100, new Color(), 1.5f);
            }
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    if (Math.Sqrt(x * x + y * y) <= radius + 0.5)   //circle
                    {
                        ...
                        {
                            Tile tile = Main.tile[xPosition, yPosition];
                            if (tile != null && Main.tile[xPosition, yPosition].liquid == 0)// && Main.tile[xPosition, yPosition].liquidType() == 0)
                            {
                                Main.tile[xPosition, yPosition].lava(false);
                                Main.tile[xPosition, yPosition].liquid = 255;
                                WorldGen.SquareTileFrame(xPosition, yPosition, true);
