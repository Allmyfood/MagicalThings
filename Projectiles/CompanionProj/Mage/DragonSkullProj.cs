...
{
    public class DragonSkullProj : ModProjectile
    {
        ...
        public override void SetDefaults()
        {
            ...
            projectile.height = 76;   //Set the hitbox heinght
            projectile.hostile = false;    //tells the game if is hostile or not.
            projectile.friendly = true;   //Tells the game whether it is friendly to players/friendly npcs or not
            projectile.ignoreWater = true;    //Tells the game whether or not projectile will be affected by water
            Main.projFrames[projectile.type] = 2;  //this is where you add how many frames u'r projectile has to make the animation
        ...
        public override void Kill(int timeLeft)
        {
            ...
            {
                int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 159, projectile.velocity.X * 1.2f, projectile.velocity.Y * 1.2f, 120, default(Color), 3.50f);   //this make so when this projectile disappear will spawn dust, change PinkPlame to what dust you want from Terraria, or add mod.DustType("CustomDustName") for your custom dust
        ...
        public override void AI()
        {
            for (int i = 0; i < 1; i++)
            {
                int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 159, projectile.velocity.X * 0.6f, projectile.velocity.Y * 0.6f, 160, default(Color), 0.8f);   //this make so when this projectile is active has dust around , change PinkFlame to what dust you want from Terraria, or add mod.DustType("CustomDustName") for your custom dust
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                {
                    if (projectile.ai[0] > 35f)//this make so the projectile1 shoot a projectile every 2 seconds(60 = 1 second so 120 = 2 seconds) 
                    {
                        ...
                        shootToX *= distance * 3;
                        shootToY *= distance * 3;
                        int damage = 40;  //this is the projectile2 damage
                        //Shoot projectile and set ai back to 0
                        {
                            Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootToX, shootToY, ProjectileID.Stake, damage, 0, Main.myPlayer, 0f, 0f);
                            //Projectile.NewProjectile(projectile.Center.X*2.5f, projectile.Center.Y*2.5f, shootToX*1.5f, shootToY, 294, damage, 0, Main.myPlayer, 0f, 0f);
                            //Projectile.NewProjectile(projectile.Center.X * -2.5f, projectile.Center.Y*-2.5f, shootToX*-1.5f, shootToY, 274, damage, 0, Main.myPlayer, 0f, 0f);
