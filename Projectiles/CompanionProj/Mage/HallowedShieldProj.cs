...
{
    public class HallowedShieldProj : ModProjectile
    {
        int shoot = ProjectileID.SwordBeam;
        ...
        {
            DisplayName.SetDefault("Hallowed Shield Attack");
        ...
        {
            Lighting.AddLight((int)(projectile.Center.X / 16f), (int)(projectile.Center.Y / 16f), 1.0f, 0.95f, 0.76f);
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                if (distance < 600f && !target.friendly && target.active && target.CanBeChasedBy(target, false))  //distance < 520 max distance to shoot to
                {
                    if (projectile.ai[0] > 8f)// time / 60fps = 0.667 times per 1 second. If time = 60 would be 1 second.
                    {
                        ...
                        int damage = 70;  //this is the projectile2 damage                   
                                          //Shoot projectile and set ai back to 0
                        Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootToX, shootToY, shoot, damage, 0, Main.myPlayer, 0f, 0f);
            ...
            projectile.ai[0] += 1f;
            if (player.dead || !player.HasBuff(mod.BuffType("HallowedArmorBuff")) || player.HasBuff(mod.BuffType("VortexShieldBuff")))
            {
                if (player.dead || !player.HasBuff(mod.BuffType("HallowedShieldBuff")) || player.HasBuff(mod.BuffType("VortexShieldBuff")))
                {
                    projectile.Kill();
                }
            }
