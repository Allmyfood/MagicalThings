...
{
    public class ValkyrieArrowProj : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            #region OnHit Make more Valkyrie Arrows
            if (projectile.owner == Main.myPlayer || mpm.ValkyrieArrowCoolDown == 0)//just to prevent crashing
            {
                Vector2 npc = Main.screenPosition + new Vector2((float)target.position.X, (float)target.position.Y);
                float ceilingLimit = npc.Y;
                if (ceilingLimit > npc.Y - 200f)
                {
                    ceilingLimit = npc.Y - 200f;
                }
                for (int i = 0; i < 24; i++)
                {
                    Vector2 position = new Vector2(0, -1);
                    position = projectile.Center + new Vector2((-(float)Main.rand.Next(-116, 116) * projectile.direction), -600f);
                    position.Y -= (50 * i);
                    Vector2 heading = npc - position;
                    if (heading.Y < 0f)
                    {
                        heading.Y *= -1f;
                    }
        ...
        public override void Kill(int timeLeft)
        {
            ...
            {
                // Create a new dust
                Dust dust = Dust.NewDustDirect(usePos, projectile.width, projectile.height, 142);
        ...
        public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
        {
            ...
            int maxStickingJavelins = 14; // This is the max. amount of javelins being able to attach
        ...
        private const float maxTicks = 975f;
        ...
        {
            Lighting.AddLight((int)(projectile.Center.X / 16f), (int)(projectile.Center.Y / 16f), 0.99f, 0.94f, 0.73f);
            ...
            if (!isStickingToTarget)
            {
                ...
                if (targetWhoAmI >= maxTicks)
                {
                    ...
                    float
                        velYmult = 0.005f; // y velocity factor, every AI update the y velocity will be be 0.35f bigger of the original speed, causing the javelin to drop to the ground
                ...
                {
                    Dust dust = Dust.NewDustDirect(projectile.position, projectile.height, projectile.width, 142,
