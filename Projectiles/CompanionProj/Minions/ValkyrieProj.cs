...
{
    public class ValkyrieProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (player.dead)
            { mpm.ValkyrieMinion = false; }

            // Stay around
            if (mpm.ValkyrieMinion)
            ...
            //int counter = 0; //use ValkyrieCoolDown instead in my player.
            bool readyup = false;
            bool cooloff = false;
            ...
            {
                AILogicMode = 1;
                mpm.ValkyrieCoolDown = 600;//keep cooldown up if npcs are around
            }
            else if (readyup == false && AILogicMode == 1 && cooloff == true)
            {
                //wait for Cooldown
            }
            else if (readyup == false && AILogicMode == 1 && cooloff == true && mpm.ValkyrieCoolDown <= 0)
            {
                cooloff = false;
            }
            else if (readyup == false && AILogicMode == 1 && cooloff == false && mpm.ValkyrieCoolDown <= 0)
            {
                AILogicMode = 0;
                mpm.ValkyrieCoolDown = 0;
            ...
            switch (AILogicMode)
            {
                case 0:
                    ...
                    if (mpm.lastNPCHitValkyrieMinion > -1)
                    {
                        target = Main.npc[mpm.lastNPCHitValkyrieMinion];
                        ...
                        { target = null; mpm.lastNPCHitValkyrieMinion = -1; }
                    }
                ...
                case 1:
                    ...
                    foreach (NPC npc in Main.npc)
                    {
                        if (npc.whoAmI == mpm.lastNPCHitValkyrieMinion) continue;
                    ...
                    if (target == null && mpm.lastNPCHitValkyrieMinion >= 0)
                    {
                        NPC npc = Main.npc[mpm.lastNPCHitValkyrieMinion];
                        Vector2 diff = npc.Center - (player.Center - new Vector2(player.direction * 64, 0));
                        float dist = Math.Max(Math.Abs(diff.X), Math.Abs(diff.Y));
                        if (dist <= closestDist && npc.CanBeChasedBy(this, false))
                        {
                            target = npc;
                            closestDist = dist;
                            mpm.lastNPCHitValkyrieMinion = -1;
                        }
                    }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            int length = Math.Min(1, 2 + (int)projectile.oldVelocity.Length());//10,2 only 1 after immage
            ...
            for (int i = length; i >= 0; i--)
            {
                ...
                if (trailOpacity > 0f)
                {
                    ...
                    spriteBatch.Draw(t,
                        ...
                        new Color(1f * colMod, 1f * colMod, 1f, 0.5f) * 1,
                        projectile.rotation,
                        origin,
                        projectile.scale * (1f + 0.02f * i),
                        effects,
                        0);//trailOpacity using 1 for full opacity instead
                }
            }
            return false;
        }
